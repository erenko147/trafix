"""
TraFix v3 — SUMO Entegrasyonlu PPO Eğitim Scripti
====================================================
...
Kullanım:
  ...
  # Özel ayarlarla
  python train_v3.py --episodes 1000 --lr 1e-4 --gui

  # Önceki checkpoint'tan devam
  python train_v3.py --resume --checkpoint coordinated_agent_weights.pth
...
class TrainConfig:
    ...
    output_dir: str = "training_outputs_v3"
    checkpoint_path: str = "coordinated_agent_weights_v3.pth"
...
class SumoEnvironment:
    ...
    def start(self, episode: int = 0):
        ...
        if not os.path.exists(cfg_path):
            raise FileNotFoundError(
                ...
                f"       python -m backend.ai.train_v3\n"
                f"  2. Yolu açıkça belirtin:\n"
                f"       python train_v3.py --sumo-cfg sumo/training.sumocfg\n"
                f"{'=' * 60}"
            )
    ...
    def get_observations(self) -> List[Dict]:
        ...
        Çıktı formatı: trafix_v3.parse_sumo_observations ile uyumlu.
        """
        observations = []
...
def train(cfg: TrainConfig):
    ...
    logging.info("=" * 60)
    logging.info("  TraFix v3 — Eğitim Başlıyor")
    logging.info("=" * 60)
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(
        description="TraFix v3 — SUMO PPO Eğitim",
