...
# Acil araç preemption ayrı bir modülde (Eren'in isteği: sim döngüsünden
# ayrık olsun, CARLA/YOLO'ya taşırken sadece o dosya değişsin).
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from emergency_preemption import EmergencyPreemptionController
...
MAX_EMERGENCY_GREEN    = 60   # adım — bu kadar sonra araç geçememiş olsa bile serbest bırak
EMERGENCY_STUCK_SPEED  = 0.1  # m/s altında araç "sıkışmış" sayılır
EMERGENCY_PUSH_AFTER   = 10   # bu kadar adım hareketsiz kalırsa ambulansı zorla geçir
...
def main():
    ...
    # ── Acil araç preemption (ayrı, bağımsız modül) ───────────────────────────
    # Tüm durum makinesi + metrik toplama emergency_preemption.py içinde.
    # Sim döngüsü sadece her adım preempt.update() çağırır.
    preempt = EmergencyPreemptionController(
        traci, tls_ids, log,
        yellow_steps=YELLOW_STEPS,
        all_red_steps=ALL_RED_STEPS,
        max_emergency_green=MAX_EMERGENCY_GREEN,
        stuck_speed=EMERGENCY_STUCK_SPEED,
        min_green_through=MIN_GREEN_THROUGH,
    )
    ...
    try:
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            # ── Acil araç preemption (ayrı modül — emergency_preemption.py) ─────
            # Sim döngüsü artık preemption mantığını BİLMEZ; sadece her adım
            # her kavşak için denetleyiciyi tetikler. CARLA/YOLO'ya taşırken
            # bu döngü değişmez, sadece o modülün algılama/aktüatör katmanı
            # değişir. Tamamlanan metrikler aşağıda backend'e POST edilir.
            for tls_id in tls_ids:
                preempt.update(
                    tls_id, step,
                    yellow_remaining=yellow_remaining,
                    pending_targets=pending_targets,
                    last_phase_change_step=last_phase_change_step,
                )

            # Tamamlanan acil-durum olaylarını backend'e gönder (dashboard
            # metrik tablosu için). Ağ hatası sessizce yutulur.
            _completed = preempt.pop_completed_sessions()
            if _completed:
                try:
                    ev_url = API_URL.replace("/telemetry", "/emergency_event")
                    requests.post(ev_url, json={"events": _completed}, timeout=0.5)
                except Exception:
                    pass
            # ─────────────────────────────────────────────────────────────────
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    for decision in decisions:
                        ...
                        # Preemption aktifse bu kavşak için AI kararını uygulama
                        if preempt.is_active(tls_id):
                            continue
            ...
            if not api_ok:
                ...
                if api_consecutive_failures >= FALLBACK_THRESHOLD:
                    ...
                    for tls_id in tls_ids:
                        # Preemption aktifse fallback da bu kavşağa dokunmaz
                        if preempt.is_active(tls_id):
                            continue
                        if yellow_remaining.get(tls_id, 0) > 0:
                            continue
